...
public class JumpSystem : MonoBehaviour
{
    ...
    float GetCurrentTotalJumpHeight()
    { //not genral height but instatneus height 
        //Debug.Log(jumpHeight + (extraJumps ? (extraJumpHeight * (jumpCounter- 1f)) : 0.0f));
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            isGrounded = true;
            //rb.AddForce(new Vector3(rb.velocity.x, 0, rb.velocity.z), ForceMode.VelocityChange); //to avoid euler number problem //or do not modify the velocity directly when applying gravity, as this can result in unrealistic behaviour -
            //next line is better than above in practice.
