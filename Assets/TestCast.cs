...
public class TestCast : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ...
        m_HitDetect = Physics.BoxCast(transform.position, new Vector3(0.5f, 0.5f, 0.5f), transform.up*-1, out m_Hit, transform.rotation, m_MaxDistance);
        if (m_HitDetect)
        {
    ...
    void OnDrawGizmos()
    {
        ...
        if (m_HitDetect)
        {
            ...
            Gizmos.DrawRay(transform.position + transform.up * -1, transform.up*-1 * m_Hit.distance);
        ...
        else
        {
            ...
            Gizmos.DrawRay(transform.position + transform.up*-0.5f, transform.up*-0.5f);
